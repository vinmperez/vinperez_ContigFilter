...
class vinperez_ContigFilterTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'vinperez_ContigFilter',
        ...
        for nameval in config.items('vinperez_ContigFilter'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = vinperez_ContigFilter(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_vinperez_ContigFilter_" + str(suffix)
